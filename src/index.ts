...
import { howToCommand } from './commands/how-to';
...
program.addCommand(verbsCommand);
...
program.addHelpText('after', () => {
  const sections = [
    ...
    ui.commandHelp([
      ...
      { cmd: 'solid publish --all', desc: 'Make pending drafts + unpublished pages live' },
      { cmd: 'solid history', desc: 'View version history for pages & KB' },
      { cmd: 'solid rollback', desc: 'Rollback to a previous version' },
      { cmd: 'solid deploy', desc: 'Shareable preview snapshot (does NOT publish)' },
